...
4. To show the owner of that file: ```ls -l <file_name>```  
5. To show the owner of that directory: ```ls -ld <dir_name>```  
...
2. To avoid problems if multiple administrator try to edit the file at the same time, you can edit it only with the special ```visudo``` command.
3. By default, the **/etc/sudoers** file also includes the contents of any files in the **/etc/sudoers.d** directory as part of the configuration file. i.e. you can create or remove files in the /etc/sudores.d directory to configure sudo access for users and groups.
4. To enable full **sudo** access for **user1** user, you can create the **/etc/sudoers.d/user1** file with the following content:  
```
user1 ALL=(ALL) ALL
```
where, \
...
6. you can also setup **sudo** to allow a user(ec2-user) to run a commands as another user without entering their **password**, by using the **NOPASSWD: ALL** command, i.e. ```ec2-user ALL=(ALL) NOPASSWD```
...
#### 3. Delete users:
Below cmd removes the username from **/etc/passwd** but leaves the user's home directory intact.
```
userdel user_name
```
